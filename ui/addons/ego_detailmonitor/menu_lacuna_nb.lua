...
local SCRIPTNAME = "menu_lacuna_nb.lua"
local SCRIPTPAGE = "698008001" -- max is 2147483647 (32-bit)
local LNBCATSPAGE = "698008002"
...
local function dump(o, indent)
    ...
    -- Handle the type of the current object
    if type(o) == 'table' then
        local s = '{\n'
        for k, v in pairs(o) do
            -- Format the key (handle string keys vs numeric keys)
            if type(k) ~= 'number' then 
                k = '"'..tostring(k)..'"' 
            end
            
            -- Recursively dump the value
            s = s .. spacing .. "  [" .. tostring(k) .. "] = " .. dump(v, indent + 1) .. ",\n"
        end
        return s .. spacing .. '}'
...
local config = {
	leftBar = {
		[1] = { name = ReadText(SCRIPTPAGE, 2400),	icon = "tlt_lacuna_nb",			mode = "encyclopedia",		helpOverlayID = "encyclopedia_sidebar_lacunanb",		helpOverlayText = ReadText(SCRIPTPAGE, 2401) },
...
-- ======================================================================
-- # BEGIN: LNB Functions Added
-- ======================================================================
-- Used by xpcall which is required by safeReadText()
local function errorHandler(err)
    return err
end

-- safeReadText prevents ReadText lookup failures from stopping the script
local function safeReadText(pageid, tid)
	local isFound
	-- Wrapping ReadText in xpcall is the only way I found to continue executing the
	-- script if ReadText fails on looking up the text entry
	local success, result = xpcall(function()
		return ReadText(pageid, tid)
	end, errorHandler)

	-- xpcall returns true for success even if ReadText fails so isFound used instead
	-- "=ReadText" in the result means the lookup failed
	if not string.match(result, "=ReadText") then
		isFound = true
	else
		isFound = false
		trace(string.format("safeReadText | action=tidLookup " ..
			"pageid=%d tid=%d isFound=%s", pageid, tid, isFound))
	end
...
function menu.addLNBCatsIndexes()
	menu.index["LNBCats"] = {}
	menu.index["LNBCats"].name = ReadText(SCRIPTPAGE, 99)

	for tid = 1, 100, 2 do
		local success, result = safeReadText(LNBCATSPAGE, tid)

		if success then
			table.insert(menu.index["LNBCats"], {key = result})
		else
			break
		end
	end
...
function menu.addLNBEntries()
	for category, _ in pairs(menu.data["LNBCats"]) do
		local catpage = menu.data["LNBCatsIndex"][category].page_id
		for tid = 1, 100, 2 do
			local entry = {}
			local success, result = safeReadText(catpage, tid)

			if success then
				entry.id = tid
				entry.name = result
				entry.icon = ""
				table.insert(menu.data["LNBCats"][category], entry)
			else
				break
			end
		end
...
function menu.getLNBCatEntry(rowdata)
	local catpage = menu.data["LNBCatsIndex"][rowdata[2]].page_id
	local tid = rowdata[3].id + 1
	local entry = {}
	
	entry.description = ReadText(catpage, tid)
	entry.banner = ""
	entry.icon = ""
	entry.image = ""
	-- .name key is required or CreateTable() will crash!
	entry.name = rowdata[3].name
	entry.video = ""

	return entry
end

function menu.getLNBCatSize(category)
	local catpage = menu.data["LNBCatsIndex"][category].page_id
	local numentries = 0
	-- trace("menu.LNBCatSize | category=" .. category .. " catpage=" .. LNBCatPage ..
	-- 	'menu.data["LNBCatsIndex"]=' .. dump(menu.data["LNBCatsIndex"]))
...
local function init()
	...
	-- Add button to topLevelMenu
    table.insert(Helper.topLevelMenus, {
        id = "lacunanb",
        name = ReadText(SCRIPTPAGE, 8104),
        icon = "tlt_lacuna_nb",
        shortcut = "",
        menu = "LacunaNBMenu",
		helpOverlayID = "toplevel_lacunanb",
		helpOverlayText = ReadText(SCRIPTPAGE, 8107),
        param = { 0, 0 },
		canopenencyclopedia = true,
    })
end
...
function menu.onShowMenu(state)
	...
	menu.title = ReadText(SCRIPTPAGE, 2400)	-- Lacuna Notebook
	...
	elseif menu.library and menu.id then
		dump("menu.onShowMenu | menu.id=" .. menu.id .. " menu.mode=" .. menu.mode)
...
function menu.addIndexEntry(array, item, name, rowdata, indent, numentries, expanded)
	...
	elseif item then
		...
		elseif rowdata[1] == "LNBCats" then
			-- All LNB entries should be marked as read
			isunread = false
		else
...
function menu.createIndex()
	...
	-- TODO: Verify if it's actually 10 and not 11
	-- ## Index Order: 10
	-- Lacuna Notebook Categories
	numentries = menu.getNumEntries("LNBCats")
	local lnb_index = menu.addIndexEntry(index, nil, menu.index["LNBCats"].name, "LNBCats", 0, numentries, menu.expanded["LNBCats"])

	for _, entry in ipairs(menu.index["LNBCats"]) do
		local cat = entry.key
		numentries = #menu.data["LNBCats"][cat]
		local lnbcat_index = menu.addIndexEntry(lnb_index, nil, entry.key, { "LNBCats", cat }, 1, numentries, menu.expanded[cat])
		for _, entry in ipairs(menu.data["LNBCats"][cat]) do
			menu.addIndexEntry(lnbcat_index, entry.id, entry.name, { "LNBCats", cat, entry }, 2)
		end
	end
...
function menu.display()
	...
	if menu.encyclopediaMode == "encyclopedia" and menu.mode ~= "LNBCats" then
		...
		row[1]:setColSpan(6):createText(ReadText(SCRIPTPAGE, 9000), Helper.headerRow1Properties)	-- "Encyclopedia + Lacuna Notebook"
	...
	-- Separate table layouts are used for entries under LNBCats
	elseif menu.encyclopediaMode == "encyclopedia" and menu.mode == "LNBCats" then
	-- ======================================================================
	-- # LNB Entries Layout - Tables
	-- Tables Layout:
	...
	-- ======================================================================
		...
		--print("framewidth: " .. framewidth .. ", tablewidth: " .. tablewidth)
		-- ======================================================================
        -- ## LNB Entries Layout - Index Table
		-- Index Table Column Layout:
		-- [ 1 ] [    2   ] [   3    ] [   4    ] [      5      ] [6]
		-- [Menu Title                                              ]
		-- [Search Box                                          ]
		-- [+/-] [Indent-0]                       [Entry Count  ]
		-- [+/-]            [Indent-1]            [Entry Count  ]
		--                             [Indent-2] [Entry Count  ]
		-- ======================================================================
		-- set up a new table
		table_index = menu.frame:addTable( 6, { width = tablewidth * 2 - Helper.borderSize, height = frameheight - topborder, x = Helper.frameBorder + menu.sidebarWidth + Helper.borderSize, y = yoffset + topborder, tabOrder = 1, maxVisibleHeight = frameheight - yoffset - topborder } )
		if menu.setdefaulttable then
		...
		-- ======================================================================
        -- ## LNB Entries Layout - Description Table (Center)
		...
		-- ======================================================================
		-- ======================================================================
		-- TODO: The commented code below is related to adding a 2nd right-margin
		--       column that narrows the description text area.  I was using
		--       setColWidthPercent to set the width but that broke display of
		--       long lines of text where random newlines would get entered in.
		--       Right now everything goes into 1 column and works as expected.
		-- ======================================================================
		table_desc = menu.frame:addTable( 1, { maxVisibleHeight = frameheight - topborder, y = yoffset + topborder, tabOrder = 2 } )
		table_desc.properties.x = table_index.properties.x + table_index.properties.width + Helper.borderSize * 2.5
		table_desc.properties.width = tablewidth * 3 - Helper.borderSize * 2.5
		-- Make sure column widths add up to 100
		local desc_column_width = 90
		local rmargin_column_width = 10

		-- Uncomment to add right-margin which narrows description reading area
		-- table_desc:setColWidthPercent(1, desc_column_width)
		-- table_desc:setColWidthPercent(2, rmargin_column_width)

		row = table_desc:addRow(false, {  })
		local icon = menu.detailIcon(false)
		if icon then
			row[1]:createIcon(icon, { width = Helper.headerRow1Height, height = Helper.headerRow1Height, color = menu.detailIconColor(false) })
		end
		-- setColSpan(2) extends blue border line under Entry Title text from
		--   Description column to Right-Margin column for entire table width
		-- row[1]:setColSpan(2):createText(menu.detailText, Helper.headerRow1Properties)
		row[1]:createText(menu.detailText, Helper.headerRow1Properties)

		local numdescrows = math.floor((table_desc.properties.maxVisibleHeight - row:getHeight()) / (Helper.scaleY(Helper.standardTextHeight) + Helper.borderSize))
		local descriptiontext = menu.getDescriptionText()
		-- FIXME: Width calculation is broken if right-margin column is added
		local description = GetTextLines(descriptiontext, Helper.standardFont, Helper.scaleFont(Helper.standardFont, Helper.standardFontSize), table_desc.properties.width - 2 * Helper.scaleX(Helper.standardTextOffsetx) - Helper.scrollbarWidth)

		if #description > numdescrows then
			-- Scrollbar required to display all rows needed for text
			description = GetTextLines(descriptiontext, Helper.standardFont, Helper.scaleFont(Helper.standardFont, Helper.standardFontSize), table_desc.properties.width - 2 * Helper.scaleX(Helper.standardTextOffsetx) - Helper.scrollbarWidth)
		end
		for linenum, descline in ipairs(description) do
			-- addrow(true, ) required for scrollbar & avoid insufficient vertical space error
			-- { fixed = false } required for wordwrap to function properly
			-- local row = table_desc:addRow(true, { fixed = false })
			-- row[1]:createText(descline, { wordwrap = true })
			local row = table_desc:addRow(true, {  })
			-- row[1]:createText(descline, { fontsize = 20, font = "Zekton fixed" })
			row[1]:createText(descline)
		end

		-- Empty space for right-margin
		-- row[2]:createText("", {})

		-- ======================================================================
        -- ## LNB Entries Layout - Detail Table
		-- Detail Table Layout:
		-- [     1     ]
        -- [Icon/Video ]
		-- [Detail Text]
		-- ======================================================================
		local detailtablewidth = 2 * tablewidth - Helper.borderSize
		table_detail = menu.frame:addTable( 1, { tabOrder = 5, width = detailtablewidth, x = framewidth - detailtablewidth - Helper.frameBorder, highlightMode = "off" } )

		local rendertargetproperties = { width = table_detail.properties.width, height = table_detail.properties.width, x = table_detail.properties.x, y = topborder }
		if table_detail.properties.width > (0.8 * Helper.viewHeight - topborder) then
			-- secure some space for the description table
			rendertargetproperties.width = 0.8 * Helper.viewHeight - topborder
			rendertargetproperties.height = 0.8 * Helper.viewHeight - topborder
		end
		...
		-- position in the center of the right side
		rendertargetproperties.x = table_detail.properties.x + (table_detail.properties.width - rendertargetproperties.width) / 2

		table_detail.properties.y = rendertargetproperties.y + rendertargetproperties.height

		row = table_detail:addRow(false, { fixed = true, bgColor = Color["row_title_background"] })
		row[1]:createText(ReadText(SCRIPTPAGE, 2404), Helper.headerRow1Properties)
		table_detail.properties.maxVisibleHeight = Helper.viewHeight - Helper.frameBorder - table_detail.properties.y

		-- TODO: Uncomment when I'm ready to add jump entries
		-- menu.numDetailRows = 0
		-- menu.addDetailRows(table_detail)

		local gapcenter = Helper.borderSize * 1.25
		Helper.drawLine( {x = table_desc.properties.x - gapcenter, y = topborder}, {x = table_desc.properties.x - gapcenter, y = frameheight}, nil, nil, Color["row_separator_encyclopedia"], true )
		Helper.drawLine( {x = table_detail.properties.x - gapcenter, y = topborder}, {x = table_detail.properties.x - gapcenter, y = frameheight}, nil, nil, Color["row_separator_encyclopedia"], true )
		...
		-- # TODO: Search is duplicated, consider making it shared between layouts
		-- only display markread on full index
		if menu.searchtext == nil or menu.searchtext == "" then
			local table_button = menu.frame:addTable(2, { width = tablewidth * 2 - Helper.borderSize, x = Helper.frameBorder + menu.sidebarWidth + Helper.borderSize, y = yoffset + topborder, tabOrder = 1 } )
	...
	-- debug("menu.display | action=dumpIndexMilitary index[7]=\n" .. dump(index[7]))
	-- debug("menu.display | action=dumpIndexLNBCats index[11]=\n" .. dump(index[11]))
	-- debug('menu.display | action=dumpDataWeapons menu.data["Weapons"]=\n' .. dump(menu.data["Weapons"]))
	-- debug('menu.display | action=dumpDataLNBCats menu.data["LNBCats"]=\n' .. dump(menu.data["LNBCats"]))
...
function menu.getDescriptionText()
	-- trace("menu.getDescriptionText | menu.mode=" .. dump(menu.mode) .. 
	-- 	" menu.library=" .. dump(menu.library) .. " menu.object=\n" .. dump(menu.object))
...
function menu.setObject(rowdata)
	...
	if type(rowdata) == "table" then
		...
		if #rowdata >= 3 then
			...
			elseif menu.mode == "LNBCats" then
				menu.library = rowdata[2]
				-- The id is retrieved from the game engine for other categories, for
				-- LNB entries a unique string is created instead
				menu.id = string.format("%s_%s_%s", rowdata[1], rowdata[2], rowdata[3].id)
				menu.object = menu.getLNBCatEntry(rowdata)
				menu.details = rowdata[3]
			else
				menu.library = rowdata[2]
...
function menu.buttonToggleEncyclopediaMode(mode)
	...
	elseif mode == "timeline" then
		Helper.closeMenuAndOpenNewMenu(menu, "LNBTimelineMenu", { 0, 0 }, true)
		menu.cleanup()
	elseif mode == "shipcomparison" then
		Helper.closeMenuAndOpenNewMenu(menu, "LNBShipComparisonMenu", { 0, 0 }, true)
		menu.cleanup()
	end
end
...
function menu.getNumEntries(category)
	...
	for subcategory in pairs(menu.data[category]) do
		if category == "LNBCats" then
			numentries = numentries + menu.getLNBCatSize(subcategory)
		elseif category ~= "Factions" then
